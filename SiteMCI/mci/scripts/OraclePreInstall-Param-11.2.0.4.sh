...
#for suse linux but as a precheck for grid 12c
sed -i  '/NOZEROCONF=yes/d' /etc/sysconfig/network
echo   'NOZEROCONF=yes' >> /etc/sysconfig/network

#duo to a bug in grid12c
sed -i  '/\/dev\/shm/d' /etc/fstab
shmsize=$(df -h| grep /dev/shm | awk '{print $2}')
echo "tmpfs           /dev/shm        tmpfs   size=${shmsize}         0 0" >> /etc/fstab
