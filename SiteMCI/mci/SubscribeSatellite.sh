satServer=satellite.idm.mci.ir
satIP=172.16.49.97
OSVersion=1
...
subscription-manager register --org="MCI" --activationkey="${activeKey}" 
if [[ $? > 1 ]]
then
	subscription-manager register --org="MCI" --activationkey="${activeKey}" --force
	if [[ $? > 1 ]]
	then
