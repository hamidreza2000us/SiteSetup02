...
#1- create a second network and first port of baremetal nodes should be connected to this network (unsure about other ports)
#2- assign at least 40GiB to each machine (tested with 500Gib)
...
#6- for 4 network (not prov and public)inside vmware vlan id should be 4095 (all)
...
#7-1 for other network you can assign accept on promiscuous mode
...
#10- if dvr two external networks is required for compute and controller
#os	ens161	ens192	ens193	ens224	ens225	ens256
#vm	4		1		5		2		6		3
#vl untag	tag		tag		public	---		public
#tag and untag network promiscuous, mac change, forged transmit = allow , mtu 1500 tested

#multiple link
#service 	controller		compute		dvr		ceph
#provi		nic1			nic1		nic1	nic1    notag
#ext		nic2						nic6			public
#interipa	nic3			nic3		nic4			tagged
#storage	nic4			nic4		nic2	nic4	tagged
#storagemg	nic5								nic5	tagged
#tenant		nic6			nic6		nic5			tagged
...
cd /home/stack/templates/nic-configs/bond-with-vlans/
sed -i 's_../../scripts/run-os-net-config.sh_/usr/share/openstack-tripleo-heat-templates/network/scripts/run-os-net-config.sh_g ' *

##########################################
#if you plan to use a dedicated bond interface for external network on controllers:
cat >  /home/stack/templates/nic-configs/bond-with-vlans/controller.patch << EOF
--- controller.yaml     2021-11-03 00:47:54.576241212 +0330
+++ controller.yaml.extbond     2021-11-03 00:47:54.576241212 +0330
@@ -127,26 +127,6 @@
                     name: nic3
                 - type: vlan
                   vlan_id:
...
EOF
cd /home/stack/templates/nic-configs/bond-with-vlans/
cp controller.yaml controller.yaml.bkp
patch controller.yaml controller.patch
##########################################
cat >  /home/stack/templates/nic-configs/bond-with-vlans/compute.patch << EOF
--- compute.yaml        2021-11-03 01:21:34.843697476 +0330
+++ compute.yaml.extbond        2021-11-03 01:23:13.355127634 +0330
@@ -146,6 +146,17 @@
                   addresses:
                   - ip_netmask:
                       get_param: TenantIpSubnet
...
EOF
cd /home/stack/templates/nic-configs/bond-with-vlans/
cp compute.yaml compute.yaml.bkp
patch compute.yaml compute.patch
##########################################
...
sed -i 's_single-nic-vlans_bond-with-vlans_g' /home/stack/templates/32-network-environment.yaml

######################################################
#if using dvr 
cp /usr/share/openstack-tripleo-heat-templates/environments/neutron-ovs-dvr.yaml /home/stack/templates/36-neutron-ovs-dvr.yaml
# 
sed -i 's_../docker_/usr/share/openstack-tripleo-heat-templates/docker_g' /home/stack/templates/36-neutron-ovs-dvr.yaml
######################################################
...
#  - /home/stack/templates/36-neutron-ovs-dvr.yaml
...
/home/stack/templates/30-network-isolation.yaml
#############################################################for bond network 
cp /usr/share/openstack-tripleo-heat-templates/ci/environments/network/public-bond/network-isolation.yaml \
/home/stack/templates/30-network-isolation.yaml
sed -i 's_../network_/home/stack/openstack-tripleo-heat-templates-rendered/network_g' \
/home/stack/templates/30-network-isolation.yaml
#############################################################
