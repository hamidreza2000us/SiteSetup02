...
mkdir /mnt/sat
mount -o loop,ro /var/Mount/ISOs/satellite-6.9.1-rhel-7-x86_64-dvd.iso /mnt/sat
...
################################################################
#there is a bug in mismatch package between rh7.9 ans sat6.9.1
yum remove -y selinux-policy-3.13.1-268.el7.noarch
#scp /var/Mount/Files/selinux* sat:~/
cd /var/Mount/Files
yum localinstall -y selinux-policy-*
################################################################
#desired time 4m12.608s
cd /mnt/sat/ &&  time ./install_packages
#expected time 18m34.467s
...
#about 10 mins
time satellite-installer --scenario satellite --foreman-proxy-tftp true --foreman-proxy-tftp-servername $(hostname)  \
--foreman-proxy-realm true --foreman-proxy-realm-principal foremanuser@${Domain^^} 
...
To update the certificates on a currently running Red Hat Satellite installation, run:
    ...
    satellite-installer --scenario satellite \
                      --certs-server-cert "/etc/pki/tls/certs/sat.idm.mci.ir.crt" \
                      --certs-server-key "/etc/pki/tls/private/sat.idm.mci.ir.key" \
                      --certs-server-ca-cert "/etc/ipa/ca.crt" \
