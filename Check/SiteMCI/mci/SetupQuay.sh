...
wget https://satellite.idm.mci.ir/pub/RHEL/Containers/mysql-57-rhel7.tar.gz
wget https://satellite.idm.mci.ir/pub/RHEL/Containers/quay_v3.3.0.tar.gz
wget https://satellite.idm.mci.ir/pub/RHEL/Containers/redis-32-rhel7.tar.gz
...
#prepare repository for ceph dockerized install
mkdir /tmp/list
cd /tmp/list
imageSource="https://satellite.idm.mci.ir/pub/RHEL/Containers/rhceph/"
quayAdmin=rhceph
quayUser=admin
quayPassword=Iahoora@123
wget --no-parent -r --reject "index.html*" ${imageSource}
cd ${imageSource#????????}
podman login $(hostname -f) -u ${quayUser} -p ${quayPassword}
for i in $(ls) ; do podman load -i $i ; done
while read i 
do 
  name=$(echo $i| awk '{print $1}') ; 
  tag=$(echo $i| awk '{print $2}') ; 
  podname=$(echo $name | awk -F/ '{print $3}') ; 
  podman tag $name:$tag $(hostname -f)/${quayAdmin}/$podname:$tag  ; 
  sleep 5
  podman push  $(hostname -f)/${quayAdmin}/$podname:$tag; 
  #echo "push is finished    >>>>>>   $(hostname -f)/rhceph/$podname:$tag"
  podman rmi $name:$tag
  podman rmi $(hostname -f)/${quayAdmin}/$podname:$tag
done < <(podman images | grep -e quay.myhost.com)
rm -rf /tmp/list
