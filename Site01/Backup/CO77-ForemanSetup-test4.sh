...
foreman-installer --scenario katello --foreman-proxy-realm true --foreman-proxy-realm-principal foremanuser@MYHOST.COM \
--foreman-initial-organization myorg \
--foreman-cli-foreman-url 'https://foreman3.myhost.com' \
--foreman-cli-username admin \
--foreman-cli-password ahoora  --foreman-initial-admin-password ahoora \
...
--foreman-proxy-tftp-servername foreman3.myhost.com
...
#foreman-maintain health check -y
...
echo "url = 'https://foreman3.myhost.com'" >> /etc/ansible/ansible.cfg
...
hammer repository   create  --name BaseOS    --content-type yum  --organization-id 1  \
--product CentOS --url http://foreman3.myhost.com/pub/media/Centos7.7 --download-policy immediate --mirror-on-sync false
hammer repository synchronize  --organization-id 1 --product CentOS  --name BaseOS #--async
###############################
#curl --insecure --output katello-ca-consumer-latest.noarch.rpm  https://foreman3.myhost.com/pub/katello-ca-consumer-latest.noarch.rpm
...
--dhcp foreman3.myhost.com  --tftp foreman3.myhost.com --discovery-id 1 --httpboot-id 1 --template-id 1 --domains myhost.com --organization-id 1
################################
hammer medium create --name CentOSRepo --os-family Redhat --path http://foreman3.myhost.com/pub/media/Centos7.7 --organization-id 1
...
hammer hostgroup create --name hostgroup01 --lifecycle-environment Library --subnet subnet13  \
 ...
 --content-view contentview01 --content-source foreman3.myhost.com --realm MYHOST.COM 
...
pubkey=$(curl -k https://foreman3.myhost.com:9090/ssh/pubkey)
...
hammer hostgroup set-parameter --hostgroup hostgroup01  --name subscription_manager_certpkg_url --parameter-type string --value https://foreman3.myhost.com/pub/katello-ca-consumer-latest.noarch.rpm
...
hammer template add-operatingsystem --name "MyKickstart01" --operatingsystem "myCent 7.7"
...
#hammer host create --name myhost01 --hostgroup hostgroup01 --content-source foreman3.myhost.com \
