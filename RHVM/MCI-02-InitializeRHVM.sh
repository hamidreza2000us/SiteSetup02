...
IDMIP=172.20.29.130
SATIP=172.20.29.132
RHVMIP=172.20.29.131
IDMForwarder=172.18.9.3

RHVHIP=172.20.29.129
RHVHGW=172.20.29.158
RHVHDNS=172.20.29.130
RHVHPass="ahoora"
...
DHCPInt=eth0
DHCPStartIP=172.20.29.140
DHCPEndIP=172.20.29.149
DHCPGW=172.20.29.158
DHCPNetMask=255.255.255.224
##############################################################################
cd /mnt/Mount/Yaml2
...
#netwrk is not match to the subnet
Network=${IPRange}.0
ssh -o StrictHostKeyChecking=no ${SATIP} /bin/bash << EOF
...
ssh -o StrictHostKeyChecking=no ${SATIP} /bin/bash << EOF
hammer host create --name ceph01 --hostgroup hostgroup01 --content-source ${SatHost}.${Domain}   \
--partition-table "Kickstart default single disk" --pxe-loader "PXELinux BIOS"   --organization-id 1  --location "Default Location" \
--interface="mac=48:df:37:ad:0f:c0"  --build true --enabled true --managed true \
--kickstart-repository "Red Hat Enterprise Linux 8 for x86_64 - BaseOS Kickstart 8.3" \
--lifecycle-environment "Library" --content-view "contentview01"

hammer host create --name ceph02 --hostgroup hostgroup01 --content-source ${SatHost}.${Domain}   \
--partition-table "Kickstart default single disk" --pxe-loader "PXELinux BIOS"   --organization-id 1  --location "Default Location" \
--interface="mac=48:df:37:ac:e5:f0"  --build true --enabled true --managed true \
--kickstart-repository "Red Hat Enterprise Linux 8 for x86_64 - BaseOS Kickstart 8.3" \
--lifecycle-environment "Library" --content-view "contentview01"

hammer host create --name ceph03 --hostgroup hostgroup01 --content-source ${SatHost}.${Domain}   \
--partition-table "Kickstart default single disk" --pxe-loader "PXELinux BIOS"   --organization-id 1  --location "Default Location" \
--interface="mac=48:df:37:ad:0d:c0"  --build true --enabled true --managed true \
--kickstart-repository "Red Hat Enterprise Linux 8 for x86_64 - BaseOS Kickstart 8.3" \
--lifecycle-environment "Library" --content-view "contentview01"
