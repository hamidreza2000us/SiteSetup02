...
then
	...
	then
                SVCName2=$(hostname -s)
                SVCName=${SVCName2::-1}
                SIP1=$(grep "${hostname}[[:space:]]" $configFile | awk '{print $3"/"$4}' | head -n +1)
                SGW1=$(grep "${hostname}[[:space:]]" $configFile | awk '{print $5}' | head -n +1)
                VIP1=$(grep "${hostname}-vip" $configFile | awk '{print $3}' | head -n +1)
                PIP11=$(grep "${hostname}-prv1" $configFile | awk '{print $3"/"$4}' | head -n +1)
                PGW11=$(grep "${hostname}-prv1" $configFile | awk '{print $5}' | head -n +1)
                PIP12=$(grep "${hostname}-prv2" $configFile | awk '{print $3"/"$4}' | head -n +1)
                PGW12=$(grep "${hostname}-prv2" $configFile | awk '{print $5}' | head -n +1)
                MIP1=$(grep "${hostname}-mgmt" $configFile | awk '{print $3"/"$4}' | head -n +1)
                MGW1=$(grep "${hostname}-mgmt" $configFile | awk '{print $5}' | head -n +1)
                BIP1=$(grep "${hostname}-bkp" $configFile | awk '{print $3"/"$4}' | head -n +1)
                BGW1=$(grep "${hostname}-bkp" $configFile | awk '{print $5}' | head -n +1)
                SCIP1=$(grep "${SVCName}-scan1" $configFile | awk '{print $3}' | head -n +1)
                SCIP2=$(grep "${SVCName}-scan2" $configFile | awk '{print $3}' | head -n +1)
                SCIP3=$(grep "${SVCName}-scan3" $configFile | awk '{print $3}' | head -n +1)
		DNS1='172.17.58.97'
		DNS2='172.17.58.98'
	else
...
then 
	...
	#we have a bug here in Ethernet filter
        for i in $(nmcli dev  | grep -v ^lo | grep 'ethernet' | awk '{print $1}' )
	do 
          if  [ ! -z "${SGW1}" ]; then
            echo "arping -c 1 -D -q -I $i $SGW1"
            arping -c 9 -D -q -I $i $SGW1
            if [ $? == 1 ]; then echo "$i $SGW1" ;SIF+=("$i"); fi
          fi

          if  [ ! -z "${PGW11}" ]; then
            echo "arping -c 1 -D -q -I $i $PGW11"
            arping -c 9 -D -q -I $i $PGW11
            if [ $? == 1 ]; then echo "$i $PGW11";P1IF+=("$i"); fi
          fi

          if  [ ! -z "${PGW12}" ]; then
            echo "arping -c 1 -D -q -I $i $PGW12"
            arping -c 9 -D -q -I $i $PGW12
            if [ $? == 1 ]; then echo "$i $PGW12" ;p2IF+=("$i"); fi
          fi

          if  [ ! -z "${BGW1}" ]; then
            echo "arping -c 1 -D -q -I $i $BGW1"
            arping -c 9 -D -q -I $i $BGW1
            if [ $? == 1 ]; then echo "$i $BGW1" ;BIF+=("$i"); fi
          fi

          if  [ ! -z "${MGW1}" ]; then
            echo "arping -c 1 -D -q -I $i $MGW1"
            arping -c 9 -D -q -I $i $MGW1
...
then
	...
	echo -e "$VIP1\t${hostname}-vip" >> /etc/hosts
fi
...
then
	...
	echo -e "$SCIP1\t${SVCName}-scan" >> /etc/hosts
	sed -i  "/$SCIP2/d" /etc/hosts
	echo -e "$SCIP2\t${SVCName}-scan" >> /etc/hosts
	sed -i  "/$SCIP3/d" /etc/hosts
	echo -e "$SCIP3\t${SVCName}-scan" >> /etc/hosts
fi
...
then
	...
	echo -e "$IP\t${hostname}-prv1" >> /etc/hosts
fi
...
then
	...
	echo -e "$IP\t${hostname}-prv2" >> /etc/hosts
fi
...
then
	...
	echo -e "$IP\t${hostname}-mgmt" >> /etc/hosts
fi
