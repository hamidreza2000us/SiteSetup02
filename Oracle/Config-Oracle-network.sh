...
then
  ...
  nmcli con add con-name ${ifname} type ethernet ifname ${ifname} ipv4.method manual ipv4.addresses ${IP}/${MASK} ipv4.gateway ${GW}
fi
...
read -rp "What is the the Backup IP/netmask address of this node : " choice; [[ -n "${choice}"  ]] &&  export BIP1="$choice";
...
then
	...
	if [ ${#SIF[@]} -eq 1 ]
	then
		setSingleIF ${SIF[0]} $IP $NM $SGW1
	fi
	if [ ${#SIF[@]} -eq 2 ]
	then
		setTeamIF Team-Service ${SIF[0]} ${SIF[1]} $IP $NM $SGW1
	fi
	if [ ${#SIF[@]} -gt 2 ]
	then
